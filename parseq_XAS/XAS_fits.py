...
class LCF_mu(fits.lcf.LCF):
    ...
    are used as ordinates and the attributes :param:`e` are used as abscissas.
    The fit curve resides in a new array :param:`fitLCF` as an attribute of the
    data container.

    The fit creates an attrubute :param:`lcf_params` of the data container that
    is a list of dictionaries, one dictionary per reference spectrum, with the
    following items:
    ...
    attribute :param:`lcf_result` of the data container will get fit info
...
class FunctionFit_mu(fits.functionfit.FunctionFit):
    ...
    The fit creates a few attrubutes of the data container:
    ...
    following keys: *value* (float), *lim* (2-list of *min* and *max*), *tie*
    (str, a tie expression) and *error* (float, the fitting error).

    The actual curve fitting is done by `scipy.optimize.curve_fit()`. The
    attribute :param:`ffit_result` of the data container will get fit info
    returned by `curve_fit()`.
    """
...
class EXAFSFit(fits.exafsfit.EXAFSFit):
    """
    The EXAFS Fit fits a sum of EXAFS shells to to data. The array attribute
    ...
    contains a 's0' fitting variable (multi-electron damping factor) and
    optional metavariables.

    The actual curve fitting is done by `scipy.optimize.curve_fit()`. The
    attribute :param:`exafsfit_result` of the data container will get fit info
    returned by `curve_fit()`.

    Two kinds of fitting errors are offered. TODO
    """

    name = "EXAFS fit"
    ref = "fits.html#exafs-fit-of-k-and-r"
